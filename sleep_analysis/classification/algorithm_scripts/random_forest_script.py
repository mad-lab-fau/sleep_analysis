...
from sleep_analysis.classification.ml_algorithms.random_forest import RandomForestOptuna, RandomForestPipeline
...
pipe = RandomForestPipeline(modality=modality, classification_type=classification)

train, test = dataset
rf_optuna = RandomForestOptuna(
    pipeline=pipe, score_function=sc.score, seed=seed, modality=modality, classification_type=classification
)
rf_optuna = rf_optuna.optimize(train)

optimized_pipeline = rf_optuna.optimized_pipeline_
...
file = open(
    Path(__file__)
    .parents[3]
    .joinpath("exports/pickle_pipelines/rf_" + dataset_name + "_" + "_".join(modality) + "_" + classification + ".obj"),
    "wb",
)
...
final_results.to_csv(
    ...
    .parents[3]
    .joinpath(
        "exports/results_per_algorithm/rf/rf_" + dataset_name + "_" + "_".join(modality) + "_" + classification + ".csv"
    ),
    index=True,
)
